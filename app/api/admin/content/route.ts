...
export async function GET() {
  ...
  const result = await db.query(`SELECT content, "updatedAt" FROM site_content WHERE id = 1 LIMIT 1`)
...
export async function PUT(request: Request) {
  ...
  await db.query(`INSERT INTO site_content (id, content, "updatedAt") VALUES (1, $1::jsonb, now()) ON CONFLICT (id) DO UPDATE SET content = EXCLUDED.content, "updatedAt" = now()`, [content])
