...
export async function POST(request: Request, context: { params: Promise<{ orderId: string }> }) {
  ...
  try {
    const body = await request.json().catch(() => ({})) as { whatsappOnly?: boolean }
    await sendPaidTicketNotifications(orderId, body.whatsappOnly ? { email: false, whatsapp: true } : undefined)
