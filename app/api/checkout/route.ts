...
export async function POST(request: Request) {
  ...
  try {
    const body = await readJson<{ name?: unknown; email?: unknown; whatsapp?: unknown; quantity?: unknown; payment_method?: unknown }>(request, 16_384); const name = String(body.name || '').trim(); const email = String(body.email || '').trim(); const whatsapp = String(body.whatsapp || '').replace(/[^\d+]/g, ''); const quantity = Number(body.quantity)
    ...
    const order = await createPayKitaOrder({ reference, name, email, whatsapp, quantity, amount: pricing.total, paymentMethod: body.payment_method })
