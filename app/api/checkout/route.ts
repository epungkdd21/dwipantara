...
import { calculatePaymentUniqueCode, calculateTicketTotal } from '@/lib/pricing'
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const nominal = Math.round(Number(pricing.total) + uniqueCode)
    ...
    return NextResponse.json({ ...order, pay_amount: nominal, unique_code: uniqueCode, ticket_codes: ticketCodes, ticket_price: pricing.rule.basePrice, pricing_label: pricing.rule.label, bundle_breakdown: pricing.bundleBreakdown })
