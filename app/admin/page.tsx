...
export default function AdminPage() {
  ...
  return <main className="min-h-screen bg-background px-4 py-6 text-foreground sm:px-6 lg:px-10">
    <header className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4"><div><div className="mb-3 flex flex-wrap items-center gap-4"><Link href="/" className="text-sm font-bold text-muted-foreground transition-colors hover:text-foreground">← Kembali ke halaman awal</Link><Link href="/sign-in" className="text-sm font-bold text-muted-foreground transition-colors hover:text-foreground">Login admin</Link></div><p className="eyebrow">DWIPANTARA 2026 · OPERATIONS</p><h1 className="mt-2 font-serif text-3xl font-bold sm:text-4xl">Dashboard {dashboard.role === 'admin' ? 'Admin' : 'Operator'}</h1><p className="mt-1 text-sm text-muted-foreground">Kelola acara, tiket, absensi, dan souvenir dari satu tempat.</p></div><button onClick={signOut} className="inline-flex items-center gap-2 rounded-full border border-border px-4 py-2 text-sm font-bold"><LogOut size={16} /> Keluar</button></header>
