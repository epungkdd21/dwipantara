...
import { ArrowRight, CalendarDays, Check, Clock3, Mail, MapPin, Menu, QrCode, Ticket, X } from 'lucide-react'
import { AmbientSound } from '@/components/ambient-sound'
import { calculateTicketTotal, getActivePricingRule, getPromoText } from '@/lib/pricing'
import { PAYMENT_METHODS, type PaymentMethodCode } from '@/lib/paykita'
...
export default function Page() {
  ...
  const [form, setForm] = useState<{ name: string; email: string; whatsapp: string; quantity: string; payment_method: PaymentMethodCode | ''; ewallet_phone: string }>({ name: '', email: '', whatsapp: '', quantity: '1', payment_method: '', ewallet_phone: '' })
  ...
  const ticketQuantity = Math.max(1, Math.min(10, Number(form.quantity) || 1))
  const checkoutTotal = calculateTicketTotal(ticketQuantity).total
