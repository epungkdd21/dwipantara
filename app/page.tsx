...
export default function Page() {
  ...
  return <main className="min-h-screen overflow-hidden bg-background text-foreground">
    ...
    <section id="home" className="batik-hero relative flex min-h-[720px] items-end bg-cover bg-center pt-24" style={{ backgroundImage: `linear-gradient(180deg, rgba(25,44,31,.08), rgba(25,44,31,.88)), url(${BG})` }}>
      <div className="mx-auto w-full max-w-6xl px-5 pb-20 lg:px-8"><p className="mb-5 font-mono text-xs font-bold uppercase tracking-[.35em] text-accent">{content.eventName ?? "Jagat 'Arsy Student Cabinet (JASCA) 2025/2026"}</p><h1 className="max-w-3xl font-serif text-5xl font-bold leading-[.98] text-primary-foreground md:text-7xl">{content.heroTitle ?? 'The beauty and harmony of Nusantara.'}</h1><p className="mt-6 max-w-xl text-lg leading-8 text-primary-foreground/85">{content.heroDescription ?? 'Satu panggung untuk merayakan ragam cerita, gerak, bunyi, dan rasa yang tumbuh dari kepulauan Indonesia.'}</p><div className="mt-9 flex flex-wrap gap-4"><a href="#tiket" className="inline-flex items-center gap-2 rounded-full bg-accent px-6 py-3 font-bold text-accent-foreground hover:bg-accent/90">Dapatkan Tiket <ArrowRight size={18} /></a><a href="#acara" className="rounded-full border border-primary-foreground/50 px-6 py-3 font-bold text-primary-foreground hover:bg-primary-foreground/10">Lihat Acara</a></div></div>
    </section>
