...
export default function Page() {
  ...
  return <main className="min-h-screen overflow-hidden bg-background text-foreground">
    ...
    <section id="home" className="batik-hero relative flex min-h-[720px] items-end bg-cover bg-center pt-24" style={{ backgroundImage: `linear-gradient(180deg, rgba(25,44,31,.08), rgba(25,44,31,.88)), url(${BG})` }}>
      <div className="mx-auto w-full max-w-6xl px-5 pb-20 lg:px-8"><p className="mb-5 font-mono text-xs font-bold uppercase tracking-[.35em] text-accent">{content.eventName ?? '꧁ ꦢ꧀ꦮꦶꦥꦤ꧀ꦠꦫ ꧂'}</p><h1 className="max-w-3xl font-serif text-5xl font-bold leading-[.98] text-primary-foreground md:text-7xl">{content.heroTitle ?? 'Festival Budaya Nusantara & Tasyakur'}</h1><p className="mt-6 max-w-xl text-lg leading-8 text-primary-foreground/85"><span className="block font-mono text-sm font-bold uppercase tracking-[.2em] text-accent">{content.heroTagline ?? 'THE BEAUTY AND HARMONY OF NUSANTARA'}</span><span className="mt-3 block">{content.heroDescription ?? "Dipersembahkan oleh Jagat 'Arsy Student Cabinet (JASCA) & Menyemarakkan Maulid Emas Ke-50 Kyai Amiin."}</span></p><div className="mt-9 flex flex-wrap gap-4"><a href="#tiket" className="inline-flex items-center gap-2 rounded-full bg-accent px-6 py-3 font-bold text-accent-foreground hover:bg-accent/90">Pesan Tiket Sekarang <ArrowRight size={18} /></a><a href="#tentang" className="rounded-full border border-primary-foreground/50 px-6 py-3 font-bold text-primary-foreground hover:bg-primary-foreground/10">Pelajari Selengkapnya</a></div></div>
    </section>

    <section id="tentang" className="mx-auto max-w-6xl px-5 py-24 lg:px-8"><div className="grid gap-14 md:grid-cols-[.8fr_1.2fr] md:items-start"><div><p className="eyebrow">01 / Tentang</p><h2 className="section-title">{content.aboutTitle ?? 'Pagelaran Seni & Budaya'}</h2><p className="mt-4 font-serif text-2xl font-bold text-primary">Tentang DWIPANTARA 2026</p></div><div className="max-w-xl space-y-5 text-lg leading-8 text-muted-foreground"><p>{content.aboutDescription ?? "DWIPANTARA merupakan festival budaya tahunan yang menghadirkan keberagaman seni, tradisi, dan kreativitas Nusantara dalam satu panggung pementasan istimewa."}</p><p>Tahun ini, DWIPANTARA diselenggarakan dalam rangka menyemarakkan Maulid Emas ke-50 Kyai Amiin sebagai wujud rasa syukur dan penghormatan atas dedikasi serta pengabdian beliau.</p><p>Acara ini sekaligus menjadi pementasan puncak persembahan dari Jagat &apos;Arsy Student Cabinet (JASCA) Masa Bakti 2025–2026 bersama Pesantren Peradaban Dunia Jagat &apos;Arsy.</p></div></div></section>

    <section id="acara" className="bg-primary py-24 text-primary-foreground"><div className="mx-auto max-w-6xl px-5 lg:px-8"><p className="eyebrow text-accent">02 / Rangkaian acara</p><h2 className="mt-4 max-w-2xl font-serif text-4xl font-bold text-primary-foreground md:text-5xl">Atraksi Utama</h2><p className="mt-4 max-w-xl text-primary-foreground/70">Rangkaian Pementasan Seni yang merayakan cerita, gerak, musik, dan tradisi Nusantara.</p><div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3"><article className="event-card"><span aria-hidden="true" className="text-2xl">01</span><h3>Seni Teater</h3><p className="mt-2 text-accent">Drama Utama</p><p className="mt-3">Pertunjukan drama klasik Sunda bertajuk &quot;Lutung Kasarung&quot; dengan tata panggung visual yang spektakuler.</p></article><article className="event-card"><span aria-hidden="true" className="text-2xl">02</span><h3>Seni Tari</h3><p className="mt-2 text-accent">Tari Tradisional</p><p className="mt-3">Kolaborasi koreografi tarian Nusantara dari Jawa, Sumatera, Bali, hingga Papua yang disatukan secara harmonis.</p></article><article className="event-card"><span aria-hidden="true" className="text-2xl">03</span><h3>Seni Musikal</h3><p className="mt-2 text-accent">Musikal Betawi</p><p className="mt-3">Sajian pertunjukan drama musikal &quot;Payung Fantasi&quot; yang membawakan nuansa nostalgia kebudayaan masyarakat Betawi.</p></article><article className="event-card"><span aria-hidden="true" className="text-2xl">04</span><h3>Atraksi Budaya</h3><p className="mt-2 text-accent">Silat &amp; Debus</p><p className="mt-3">Demonstrasi ketangkasan pencak silat tradisional serta atraksi pementasan seni kekebalan khas Tanah Banten.</p></article><article className="event-card border-accent/60"><span aria-hidden="true" className="text-2xl">05</span><h3>Puncak Acara</h3><p className="mt-2 text-accent">Maulid Emas 50 Tahun</p><p className="mt-3">Prosesi tasyakur dan penghormatan penuh makna atas Milad Ke-50 Kyai Amiin dirangkaikan dengan lantunan sholawat.</p></article></div></div></section>

    <section id="lokasi" className="mx-auto max-w-6xl px-5 py-24 lg:px-8"><div className="grid gap-12 md:grid-cols-2"><div><p className="eyebrow">03 / Waktu & lokasi</p><h2 className="section-title">Datang dan<br /><em>jadilah bagian.</em></h2></div><div className="grid gap-4"><div className="info-row"><CalendarDays /><div><strong>{content.date ?? 'Sabtu, 17 Oktober 2026'}</strong><p>Sabtu</p></div></div><div className="info-row"><Clock3 /><div><strong>{content.time ?? "Ba&apos;da Isya – 22.38 WIB"}</strong><p>Pementasan malam</p></div></div><div className="info-row"><MapPin /><div><strong>Lapangan Futsal Pesantren Jagat &apos;Arsy BSD</strong><p>{content.venue ?? "Serpong, Tangerang Selatan, Banten"}</p></div></div></div></div></section>

    <section id="tiket" className="bg-secondary px-5 py-24 lg:px-8"><div className="mx-auto grid max-w-6xl gap-12 md:grid-cols-[.9fr_1.1fr] md:items-start"><div><p className="eyebrow">04 / Tiket masuk</p><h2 className="section-title">Satu tiket,<br /><em>seribu cerita.</em></h2><p className="mt-6 max-w-md leading-7 text-muted-foreground">Tiket festival berlaku untuk satu hari pilihanmu dan sudah termasuk akses seluruh pertunjukan utama.</p><div className="mt-8 flex items-center gap-3"><Ticket className="text-accent" /><span className="text-2xl font-bold">Rp{(content.ticketPrice ?? 10000).toLocaleString('id-ID')} <small className="text-sm font-normal text-muted-foreground">/ orang</small></span></div></div><form onSubmit={buyTicket} className="rounded-3xl border border-border bg-card p-6 shadow-xl md:p-8"><h3 className="mb-6 font-serif text-2xl font-bold">Pesan tiket sekarang</h3><div className="grid gap-4"><label>Nama lengkap<input required value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Nama kamu" /></label><label>Email<input required type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} placeholder="nama@email.com" /></label><label>Nomor WhatsApp<input required value={form.whatsapp} onChange={e => setForm({ ...form, whatsapp: e.target.value })} placeholder="08xxxxxxxxxx" /></label><label>Jumlah tiket<input required type="number" min="1" max="10" value={form.quantity} onChange={e => setForm({ ...form, quantity: e.target.value })} /></label></div>{error && <p className="mt-4 rounded-lg bg-destructive/10 p-3 text-sm text-destructive">{error}</p>}<button disabled={busy} className="mt-6 flex w-full items-center justify-center gap-2 rounded-full bg-primary px-5 py-3.5 font-bold text-primary-foreground disabled:opacity-60">{busy ? 'Menyiapkan pembayaran…' : 'Lanjut ke pembayaran'} <ArrowRight size={18} /></button></form></div></section>
