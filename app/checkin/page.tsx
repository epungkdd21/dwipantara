...
export default function CheckinPage() {
  ...
  function handleScannerInput(event: React.KeyboardEvent<HTMLInputElement>) {
    ...
    if (code.trim().length >= 8 && !loading) (event.currentTarget.form as HTMLFormElement | null)?.requestSubmit()
  }
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      else { setTicket(data.ticket); setMessage({ ok: true, text: 'Tiket terverifikasi. Pengunjung tercatat hadir.' }); setCode(''); await loadLogs(); requestAnimationFrame(() => codeInputRef.current?.focus()) }
